    ...
    public static class ServiceClientFactory
    {
        ...
        public static IDictionary<string, string> Cookies
        {
            get
            {
                if (m_Cookies == null)
                {
                    throw new InvalidOperationException("The cookie jar has not been initialized: Call ServiceClientFactory." +
                        " ReadCookies when the application starts");
                }
                return m_Cookies;
            }
        }

        public static void Clear(Action<bool> callback)
        {
            Deployment.Current.Dispatcher.BeginInvoke(() =>
            {
                HtmlPage.Document.Cookies += String.Format("{0}expires={1}", (HtmlPage.Document.Cookies.Length == 0 ? "": ","), DateTime.Now.ToOADate().ToString());
                //if (m_Cookies != null && m_Cookies.Keys.Contains(GreenField.Common.CookieEncription.Encript("UserName")))
                //{
                //    m_Cookies.Add("expires", DateTime.Now.ToString("R"));
                //}
                callback(true);
            });
        }
            ...
            Deployment.Current.Dispatcher.BeginInvoke(() =>
            {
                ...
                foreach (string cookie in cookieSplit)
                {
                    ...
                    if (keyAndValue.Length == 2)
                    {
                        ...
                        Double expireOADate;
                        if (cookieKey.Equals("expires") && Double.TryParse(cookieValue, out expireOADate))
                        {
                            if (expireOADate < DateTime.Now.ToOADate())
                            {
                                m_Cookies.Clear();
                                break;
                            }
