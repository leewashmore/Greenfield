...
{
    public class ViewModelPE : NotificationObject
    {
        #region Fields
        ...
        /// Stores Chart data
        /// </summary>
        private RangeObservableCollection<PRevenueData> _PEPlottedData;
        #endregion
        ...
        public ViewModelPE(DashboardGadgetParam param)
        {
            ...
            {
                _dbInteractivity.RetrievePRevenueData(_securitySelectionData, RetrievePEDataCallbackMethod);
        ...
        public RangeObservableCollection<PRevenueData> PEPlottedData
        {
            get
            {
                if (_PEPlottedData == null)
                    _PEPlottedData = new RangeObservableCollection<PRevenueData>();
                return _PEPlottedData;
            }
            set
            {
                _PEPlottedData = value;
                RaisePropertyChanged(() => this.PEPlottedData);
            }
        ...
        /// </summary>
        public event DataRetrievalProgressIndicatorEventHandler PEDataLoadEvent;
        ...
        public void HandleSecurityReferenceSet(EntitySelectionData entitySelectionData)
        {
            ...
            try
            {
                if (entitySelectionData != null && IsActive)
                {
                    ...
                    {
                        if (PEDataLoadEvent != null)
                            PEDataLoadEvent(new DataRetrievalProgressIndicatorEventArgs() { ShowBusy = true });
                        _dbInteractivity.RetrievePRevenueData(entitySelectionData, RetrievePEDataCallbackMethod);
                    }
                }
                else
                {
        ...
        private void RetrievePEDataCallbackMethod(List<PRevenueData> pRevenueData)
        {
            string methodNamespace = String.Format("{0}.{1}", GetType().FullName, System.Reflection.MethodInfo.GetCurrentMethod().Name);
            Logging.LogBeginMethod(_logger, methodNamespace);
            try
            {
                if (pRevenueData != null)
                {
                    ...
                    PEPlottedData.Clear();
                    PEPlottedData.AddRange(pRevenueData.ToList());
                }
                else
                {
                    Logging.LogMethodParameterNull(_logger, methodNamespace, 1);
