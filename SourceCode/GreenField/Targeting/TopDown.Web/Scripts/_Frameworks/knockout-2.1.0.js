...
ko.bindingHandlers['focus'] = {
        init: function (element, valueAccessor, allAcceptor, viewModel) {
            var handler = ko.utils.unwrapObservable(valueAccessor());
            if (handler != null) {
                $(element).focus(function (e) {
                    window.setTimeout(handler.bind(viewModel), 0);
                });
            }
        }
    };
    ...
    ko.bindingHandlers['doubleclick'] = {
        init: function (element, valueAccessor, allValueAccessors, viewModel) {
            var handler = ko.utils.unwrapObservable(valueAccessor() || {});
    ...
    ko.bindingHandlers['selectable'] = {
        init: function (element, valueAccessor) {
            var value = ko.utils.unwrapObservable(valueAccessor() || {});
            $(element).disableSelection(value);
        }
    };

    function stopHandler(e, ui) {
        var at = ui.position;
        this.x(at.left);
        this.y(at.top);
    }

    function stopHandlerOverriden(baseHandler, e) {
        stopHandler.call(this, e);
        baseHandler.call(this, e);
    }

    ko.bindingHandlers['draggable'] = {
        init: function (element, valueAccessor, allValueAccessor, viewModel) {
            var options = ko.utils.unwrapObservable(valueAccessor() || { track: false });

            // check if tracked is requested, here tracked means getting coordinates of the dragged
            // element back to the view model via x and y observables

            if (!!options.tracked) {
                if (!ko.isObservable(viewModel.x) || !ko.isObservable(viewModel.y)) throw new Error('There must be x and y.');
                var previousHandler = options.stop;
                if (previousHandler != null) {
                    options.stop = stopHandlerOverriden.bind(viewModel, options.stop);
                } else {
                    options.stop = stopHandler.bind(viewModel);
                }
            }
            ...
            if (handler != null) {
                $(element).keydown(function (e) {
                    if (e.which === 13) {
                        ...
                        e.stopPropagation();
                        e.preventDefault();
                        window.setTimeout(function() { element.blur(); }, 0);
