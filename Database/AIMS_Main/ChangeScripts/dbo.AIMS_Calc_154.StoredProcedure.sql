...
as
	...
	BEGIN
		...
		select pf.* 
	  --into #A
		from dbo.PERIOD_FINANCIALS_ISSUER_STAGE  pf with (nolock) --aded to prevent deadlock -- Splitting into 2 tables Change to insert the data into period_Financials_issuer
	...
	BEGIN
		...
		insert into #A
		select pf.* 
		from dbo.PERIOD_FINANCIALS_ISSUER_MAIN  pf with (nolock) --aded to prevent deadlock -- Splitting into 2 tables Change to insert the data into period_Financials_issuer
		where DATA_ID = 259			--VRUQ		
		and pf.ISSUER_ID = @ISSUER_ID
		and pf.PERIOD_TYPE = 'A'
