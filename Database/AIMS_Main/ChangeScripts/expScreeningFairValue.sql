...
select gsb.ISSUER_ID, 
	...
	isnull(adr.SHARES_PER_ADR,gsb.SHARES_PER_ADR) as SHARES_PER_ADR
into #S1	
from dbo.GF_SECURITY_BASEVIEW gsb
left join dbo.ADR_GDR_CONVERSION_OVERRIDE adr on gsb.SECURITY_ID = adr.SECURITY_ID
...
*/
case when CURRENT_MEASURE_VALUE = 0 then NULL else (price*(1+UPSIDE))/(SELL/BUY) end as BUY_PRICE,
case when CURRENT_MEASURE_VALUE = 0 then NULL else price*(1+UPSIDE) end as SELL_PRICE
from #Screening s
