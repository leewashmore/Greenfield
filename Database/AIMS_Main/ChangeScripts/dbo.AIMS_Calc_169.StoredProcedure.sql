...
alter procedure [dbo].[AIMS_Calc_169](
...
as
	  ...
	  BEGIN
			...
			from dbo.PERIOD_FINANCIALS_SECURITY_STAGE  pf with (nolock) -- Splitting into 2 tables 
	...
	-- Get the data
			...
			select pf.* 
			  ...
			  from dbo.PERIOD_FINANCIALS_SECURITY_MAIN  pf with (nolock) -- Splitting into 2 tables 
			 inner join dbo.GF_SECURITY_BASEVIEW sb on sb.SECURITY_ID = pf.SECURITY_ID
			 where DATA_ID = 185			--Market Capitalization		
			   and sb.ISSUER_ID = @ISSUER_ID
			   and pf.PERIOD_TYPE = 'A'
