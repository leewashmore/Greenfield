...
/****** Object:  StoredProcedure [dbo].[expAnalystWatchList]    Script Date: 08/06/2014 15:48:55 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER OFF
GO

alter procedure [dbo].expAnalystWatchList
	as
...
select imr.issuer_id 
	, sb.ISO_COUNTRY_CODE
	, imr.presenter as Presenter
	, sb.ASHMOREEMM_PRIMARY_ANALYST as 'Analyst'
	, sb.TICKER
	, sb.ISSUER_NAME as 'Company Name'
...
--	, imr.ic_measure
	...
	, im.CURR_ACTION as 'Analyst Tracking Parameter'
	, im.UPDATE_DATE as 'Update Date'
	, im.DATA_SOURCE as 'Data Source'
	, isnull(pp.PCT_PORTFOLIO,0) as '%Port'
	, pp.PORPATH as 'Portfolio'
...
and imr.presenter in ('MADERC','CAHOONM','AGARWALA','CHENY','CHIC','DAGUIARB','DORSONM','GARZAA','LEEG','LEVEILLEE','MOSESA','NAGS','SHUMELDAT','TALWARR','TANTALOJ','YUANJ')
and (im.DATA_SOURCE = 'PRIMARY' or im.DATA_SOURCE is null)
and pp.PCT_PORTFOLIO IS NULL
and imr.date_presented > (GETDATE() - 366)
ORDER BY 'IC Upside' DESC, Presenter, ISSUER_NAME DESC
...
order by at.Analyst, 'IC Upside' DESC
