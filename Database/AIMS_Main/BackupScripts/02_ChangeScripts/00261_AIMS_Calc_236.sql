...
declare @RequiredDBVersion as nvarchar(100) = '00260'
declare @CurrentScriptVersion as nvarchar(100) = '00261'
...
GO
            ------------------------------------------------------------------------
-- Purpose:	This procedure calculates the value for DATA_ID:236 Forward Dividend Yield
--
--			:   Sum of next 4 quarters FCDP**/ Current Market Cap*
--
-- Author:	Shivani 
-- Date:	July 18, 2012
------------------------------------------------------------------------
IF OBJECT_ID ( 'AIMS_Calc_236', 'P' ) IS NOT NULL 
DROP PROCEDURE AIMS_Calc_236;
GO

CREATE procedure [dbo].[AIMS_Calc_236](
...
as
 ...
 ---- Total amount for all the fiscal quarters within an year --- 
	...
	select distinct a.ISSUER_ID, a.SECURITY_ID, a.COA_TYPE, a.DATA_SOURCE, a.ROOT_SOURCE
		...
		,  case when c.AMOUNT is NULL then b.AMOUNT100 else (b.AMOUNT +c.AMOUNT) end /a.AMOUNT as AMOUNT	 					--  Sum of next 4 quarters FCDP**/Current Market Cap* 
		,  case when c.AMOUNT is NULL then  'Dividends(' + CAST(b.AMOUNT100 as varchar(32)) + ') / Mktcap(' + CAST(a.AMOUNT as varchar(32)) + ')' 
				else 'Dividends(' + CAST(b.AMOUNT as varchar(32)) + ' + ' +CAST(c.AMOUNT as varchar(32)) + ') / Mktcap(' + CAST(a.AMOUNT as varchar(32)) + ')' end as CALCULATION_DIAGRAM
		,  a.SOURCE_CURRENCY
		,  a.AMOUNT_TYPE
	  from #A a
