...
declare @RequiredDBVersion as nvarchar(100) = '00237'
declare @CurrentScriptVersion as nvarchar(100) = '00238'
...
--
--			:   Sum of previous 4 quarters NINC**/Previous Annual QTLE*
--
-- Author:	Anupriya
-- Date:	July 19, 2012
------------------------------------------------------------------------
...
as
 ...
 ---- Total amount for all the fiscal quarters within an year --- 
	...
	select  distinct a.ISSUER_ID, a.SECURITY_ID, a.COA_TYPE, a.DATA_SOURCE, a.ROOT_SOURCE
		...
		,  b.AMOUNT /a.AMOUNT as AMOUNT						-- Sum of Previous 4 quarters NINC**/Previous Annual QTLE*
		,  'Sum of 4 quarters(' + CAST(b.AMOUNT as varchar(32)) + ') / Previous Annual NINC(' + CAST(a.AMOUNT as varchar(32)) + ')' as CALCULATION_DIAGRAM
		,  a.SOURCE_CURRENCY
		,  a.AMOUNT_TYPE
	  from #A a
	...
	if @CALC_LOG = 'Y'
		BEGIN	
			...
			select GETDATE() as LOG_DATE, 212 as DATA_ID, a.ISSUER_ID, 'C'
				...
				, 'ERROR calculating 212 Trailing ROE. DATA_ID:104 QTLE is NULL or ZERO'
			  from #A a
			...
			select GETDATE() as LOG_DATE, 212 as DATA_ID, a.ISSUER_ID, 'C'
				, 0, '01/01/1900', a.FISCAL_TYPE, a.CURRENCY
				, 'ERROR calculating 212 Trailing ROE.  DATA_ID:104 QTLE is missing' as TXT
			  from #A a
			...
			select GETDATE() as LOG_DATE, 212 as DATA_ID, isnull(@ISSUER_ID, ' ') as ISSUER_ID, ' ' as PERIOD_TYPE
				...
				, 'ERROR calculating 212 Trailing ROE .  DATA_ID:104 QTLE no data' as TXT
			  from (select COUNT(*) CNT from #A having COUNT(*) = 0) z
			) union (	

			select GETDATE() as LOG_DATE, 212 as DATA_ID, isnull(@ISSUER_ID, ' ') as ISSUER_ID, ' ' as PERIOD_TYPE
				,  0 as PERIOD_YEAR,  '1/1/1900' as PERIOD_END_DATE,  ' ' as FISCAL_TYPE,  ' ' as CURRENCY
				, 'ERROR calculating 212 Trailing ROE.  DATA_ID:44 NINC  No data or missing quarters' as TXT
