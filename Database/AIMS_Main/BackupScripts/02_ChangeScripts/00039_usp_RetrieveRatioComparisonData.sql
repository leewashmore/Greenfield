	...
	IF @RatioDataCount <> 0
		BEGIN
			...
			WHERE (PF.SECURITY_ID = SI.SECURITY_ID OR PF.ISSUER_ID = SI.ISSUER_ID)
				...
				AND PF.FISCAL_TYPE = 
					CASE WHEN @PeriodYear IS NULL THEN PF.FISCAL_TYPE 
					ELSE 'FISCAL' END
				AND PF.PERIOD_YEAR = ISNULL(@PeriodYear, PF.PERIOD_YEAR)
			...
			WHERE (CCE.SECURITY_ID = SI.SECURITY_ID OR CCE.ISSUER_ID = SI.ISSUER_ID)
				...
				AND CCE.FISCAL_TYPE = 
					CASE WHEN @PeriodYear IS NULL THEN CCE.FISCAL_TYPE 
					ELSE 'FISCAL' END
				AND CCE.PERIOD_YEAR = ISNULL(@PeriodYear, CCE.PERIOD_YEAR)
