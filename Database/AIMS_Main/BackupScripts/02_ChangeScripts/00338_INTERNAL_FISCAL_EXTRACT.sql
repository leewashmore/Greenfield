...
declare @RequiredDBVersion as nvarchar(100) = '00337'
declare @CurrentScriptVersion as nvarchar(100) = '00338'
...
--modified 1/29/13 (JM):  this should not be needed as internal_issuer should be populated on the init of get_data for each issuer
	...
	Select i.*, i.CURRENCY as SOURCE_CURRENCY, id.PERIOD_TYPE, id.AMOUNT * (12.000000/ISNULL(i.PERIOD_LENGTH,12.000000)) as AMOUNT, dm.DATA_ID, dm.COA
	 ...
	 inner join #DM dm on dm.COA = id.COA		-- limits the data collected
	  left join dbo.FX_RATES fxs on fxs.CURRENCY = i.CURRENCY and fxs.FX_DATE = i.PERIOD_END_DATE
	 where i.ISSUER_ID = @ISSUER_ID
	  ...
	  and isnull(fxs.AVG12MonthRATE, 0.0) <> 0.0   --only pull if we have fx rates to convert it
