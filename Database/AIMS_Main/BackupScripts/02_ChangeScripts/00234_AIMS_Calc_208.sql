...
declare @RequiredDBVersion as nvarchar(100) = '00233'
declare @CurrentScriptVersion as nvarchar(100) = '00234'
...
GO
            ------------------------------------------------------------------------
-- Purpose:	This procedure calculates the value for DATA_ID:208 Trailing EV/EBITDA
--
--			Current Enterprise Value* / Sum of previous 4 quarters EBITDA (130)**
--
-- Author:	Anupriya
-- Date:	July 18, 2012
------------------------------------------------------------------------
IF OBJECT_ID ( 'AIMS_Calc_208', 'P' ) IS NOT NULL 
DROP PROCEDURE AIMS_Calc_208;
GO

CREATE procedure [dbo].[AIMS_Calc_208](
...
as
 ...
 ---- Total amount for all the fiscal quarters within an year --- 
	...
	select  distinct a.ISSUER_ID, a.SECURITY_ID, a.COA_TYPE, a.DATA_SOURCE, a.ROOT_SOURCE
		...
		,  208 as DATA_ID										-- DATA_ID:208 Trailing EV/EBITDA
		,  a.AMOUNT /b.AMOUNT as AMOUNT						-- Current Market Cap* / Sum of previous 4 quarters EBITDA**
	...
	if @CALC_LOG = 'Y'
		BEGIN
			...
			(
			select GETDATE() as LOG_DATE, 208 as DATA_ID, a.ISSUER_ID, 'C'
				,   0, '01/01/1900', a.FISCAL_TYPE, a.CURRENCY
				, 'ERROR calculating 208 Trailing EV/EBITDA. DATA_ID:130 EBITDA is NULL or ZERO'
			 from #B a
			...
			select GETDATE() as LOG_DATE, 208 as DATA_ID, a.ISSUER_ID, 'C'
				,  0, '01/01/1900', a.FISCAL_TYPE, a.CURRENCY
				, 'ERROR calculating 208 Trailing EV/EBITDA .  DATA_ID:130 EBITDA  is missing' as TXT
			  from #A a
			 inner join dbo.GF_SECURITY_BASEVIEW sb on sb.SECURITY_ID = a.SECURITY_ID
			...
			select GETDATE() as LOG_DATE, 208 as DATA_ID, a.ISSUER_ID, 'C'
				,  0, '01/01/1900', a.FISCAL_TYPE, a.CURRENCY
				, 'ERROR calculating 208 Trailing EV/EBITDA .  DATA_ID:186 Enterprise Value is missing' as TXT
			  from #B a
			...
			select GETDATE() as LOG_DATE, 208 as DATA_ID, isnull(@ISSUER_ID, ' ') as ISSUER_ID, ' ' as PERIOD_TYPE
				,  0 as PERIOD_YEAR,  '1/1/1900' as PERIOD_END_DATE,  ' ' as FISCAL_TYPE,  ' ' as CURRENCY
				, 'ERROR calculating 208 Trailing EV/EBITDA .  DATA_ID:186 no data' as TXT
			  from (select COUNT(*) CNT from #A having COUNT(*) = 0) z
			) union (	

			select GETDATE() as LOG_DATE, 208 as DATA_ID, isnull(@ISSUER_ID, ' ') as ISSUER_ID, ' ' as PERIOD_TYPE
				,  0 as PERIOD_YEAR,  '1/1/1900' as PERIOD_END_DATE,  ' ' as FISCAL_TYPE,  ' ' as CURRENCY
				, 'ERROR calculating 208 Trailing EV/EBITDA.  DATA_ID:130 EBITDA No data or missing quarters' as TXT
