...
declare @RequiredDBVersion as nvarchar(100) = '00223'
declare @CurrentScriptVersion as nvarchar(100) = '00224'
...
GO
            ------------------------------------------------------------------------
-- Purpose:	This procedure calculates the value for DATA_ID:198 Forward EV/EBITDA
--
--			:   Current Enterprise Value* / Sum of next 4 quarters EBITDA (130)**
--
-- Author:  Shivani
-- Date:	July 17, 2012
------------------------------------------------------------------------
IF OBJECT_ID ( 'AIMS_Calc_198', 'P' ) IS NOT NULL 
DROP PROCEDURE AIMS_Calc_198;
GO
...
as
	...
	select distinct a.ISSUER_ID, a.SECURITY_ID, a.COA_TYPE, a.DATA_SOURCE, a.ROOT_SOURCE
		...
		,  case when c.AMOUNT is NULL then 'Ent Value(' + CAST(a.AMOUNT as varchar(32)) + ') / EBITDA(' + CAST(b.AMOUNT100 as varchar(32)) + ')' 
				else 'Ent Value(' + CAST(a.AMOUNT as varchar(32)) + ') / EBITDA(' + CAST(b.AMOUNT as varchar(32)) + ' + ' +CAST(c.AMOUNT as varchar(32)) + ')' end as CALCULATION_DIAGRAM
		,  a.SOURCE_CURRENCY
		,  a.AMOUNT_TYPE
	  from #A a
	...
	if @CALC_LOG = 'Y'
		BEGIN
			...
			select GETDATE() as LOG_DATE, 198 as DATA_ID, a.ISSUER_ID, 'C'
				...
				, 'ERROR calculating 198 Forward EV/EBITDA. DATA_ID:130 is NULL or ZERO'
			  from #B a
			 where isnull(a.AMOUNT, 0.0) = 0.0	 -- Data error	 
			) union (	
			
			-- Error conditions - missing data 
			select GETDATE() as LOG_DATE, 198 as DATA_ID, sb.ISSUER_ID, 'C'
				,  0, '01/01/1900', a.FISCAL_TYPE, a.CURRENCY
			...
			select GETDATE() as LOG_DATE, 198 as DATA_ID, sb.ISSUER_ID, 'C'
				,  0, '01/01/1900', a.FISCAL_TYPE, a.CURRENCY
				, 'ERROR calculating 198 Forward EV/EBITDA .  DATA_ID:130 is missing' as TXT
			  from #A a
			 inner join dbo.GF_SECURITY_BASEVIEW sb on sb.SECURITY_ID = a.SECURITY_ID
			...
			select GETDATE() as LOG_DATE, 198 as DATA_ID, isnull(@ISSUER_ID, ' ') as ISSUER_ID, ' ' as PERIOD_TYPE
				,  0 as PERIOD_YEAR,  '1/1/1900' as PERIOD_END_DATE,  ' ' as FISCAL_TYPE,  ' ' as CURRENCY
				, 'ERROR calculating 198 Forward EV/EBITDA.  DATA_ID:130 No data or missing quarters' as TXT
