...
declare @RequiredDBVersion as nvarchar(100) = '00290'
declare @CurrentScriptVersion as nvarchar(100) = '00291'
...
GO
            ------------------------------------------------------------------------
-- Purpose:	This procedure calculates the value for DATA_ID:279 Forward Earnings

-- Sum of next 4 quarters NINC**
--
-- Author:	Aniket 
-- Date:	August 06, 2012
------------------------------------------------------------------------
IF OBJECT_ID ( 'AIMS_Calc_279', 'P' ) IS NOT NULL 
DROP PROCEDURE AIMS_Calc_279;
GO

CREATE procedure [dbo].[AIMS_Calc_279](
...
as
	...
	select distinct a.ISSUER_ID, a.SECURITY_ID, a.COA_TYPE, a.DATA_SOURCE, a.ROOT_SOURCE
		...
		,  279 as DATA_ID										-- DATA_ID: 279 Forward Earnings
		...
		,  case when b.AMOUNT is NULL then 'Earnings(' + CAST(a.AMOUNT100 as varchar(32)) + ')' 
				else 'Earnings(' + CAST(a.AMOUNT as varchar(32)) + ' + ' +CAST(b.AMOUNT as varchar(32)) + ')' end as CALCULATION_DIAGRAM
		,  a.SOURCE_CURRENCY
		,  a.AMOUNT_TYPE
	  from #A a
...
--	   and isnull(c.AMOUNT, 0.0) <> 0.0	-- Data validation
	...
	if @CALC_LOG = 'Y'
		BEGIN
			...
			(
			select GETDATE() as LOG_DATE, 279 as DATA_ID, a.ISSUER_ID, 'C'
				,  0, '01/01/1900', a.FISCAL_TYPE, a.CURRENCY
				, 'ERROR calculating 279 Forward Earnings. DATA_ID:290 is NULL or ZERO'
			  from #A a
			 where isnull(a.AMOUNT, 0.0) = 0.0	 -- Data error	 
			) union (	
			
				-- ERROR - No data at all available
			select GETDATE() as LOG_DATE, 279 as DATA_ID, isnull(@ISSUER_ID, ' ') as ISSUER_ID, ' ' as PERIOD_TYPE
				,  0 as PERIOD_YEAR,  '1/1/1900' as PERIOD_END_DATE,  ' ' as FISCAL_TYPE,  ' ' as CURRENCY
				, 'ERROR calculating 279 Forward Earnings.  DATA_ID:290 No data or missing quarters' as TXT
