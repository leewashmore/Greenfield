...
as
	...
	-- get the data for the prior year
	select pf.PERIOD_YEAR + 1 as PRIOR_YEAR, pf.PERIOD_YEAR as PERIOD_YEAR, pf.CURRENCY, pf.COA_TYPE
		,  pf.DATA_SOURCE, pf.FISCAL_TYPE, pf.PERIOD_TYPE, pf.ISSUER_ID, pf.AMOUNT, pf.PERIOD_END_DATE
		,  pf.AMOUNT_TYPE
	  into #B
	   from dbo.PERIOD_FINANCIALS pf 
	 where DATA_ID = 44			--NINC
	...
	select a.ISSUER_ID, a.SECURITY_ID, a.COA_TYPE, a.DATA_SOURCE, a.ROOT_SOURCE
		...
		,  '(NINC for ('+ CAST(a.PERIOD_YEAR as varchar(32)) + '(' + CAST(a.AMOUNT as varchar(32)) + ') / NINC for ('+ CAST(b.PERIOD_YEAR as varchar(32)) +')(' + CAST(b.AMOUNT as varchar(32)) + ')) - 1 ' as CALCULATION_DIAGRAM
		,  a.SOURCE_CURRENCY
		,  a.AMOUNT_TYPE
	  from #A a
	...
	if @CALC_LOG = 'Y'
		BEGIN	
			...
			select GETDATE() as LOG_DATE, 177 as DATA_ID, a.ISSUER_ID, a.PERIOD_TYPE
				...
				, 'ERROR calculating 177 Net Income Growth (YOY) .  DATA_ID:44 is NULL or ZERO for prior period'
			  from #B a
			...
			select GETDATE() as LOG_DATE, 177 as DATA_ID, a.ISSUER_ID, a.PERIOD_TYPE
				,  a.PERIOD_YEAR, a.PERIOD_END_DATE, a.FISCAL_TYPE, a.CURRENCY
				, 'ERROR calculating 177 Net Income Growth (YOY) .  DATA_ID:44 is missing for the prior year' as TXT
			  from #A a
			...
			select GETDATE() as LOG_DATE, 177 as DATA_ID, a.ISSUER_ID, a.PERIOD_TYPE
				,  a.PERIOD_YEAR,  a.PERIOD_END_DATE,  a.FISCAL_TYPE,  a.CURRENCY
				, 'ERROR calculating 177 Net Income Growth (YOY) .  DATA_ID:44 is missing for the year' as TXT
			  from #B a
			...
			select GETDATE() as LOG_DATE, 177 as DATA_ID, isnull(@ISSUER_ID, ' ') as ISSUER_ID, ' ' as PERIOD_TYPE
				...
				, 'ERROR calculating 177 Net Income Growth (YOY) .  DATA_ID:44 no data' as TXT
