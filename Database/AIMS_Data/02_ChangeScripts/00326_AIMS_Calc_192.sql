...
declare @RequiredDBVersion as nvarchar(100) = '00325'
declare @CurrentScriptVersion as nvarchar(100) = '00326'
...
--
--			:  Dividend (FCDP) / Market Cap (185)
--
-- Author:	Justin Machata
-- Date:	January 28, 2013
------------------------------------------------------------------------
IF OBJECT_ID ( 'AIMS_Calc_192', 'P' ) IS NOT NULL 
DROP PROCEDURE AIMS_Calc_192;
GO

CREATE procedure [dbo].[AIMS_Calc_192](
...
as
	...
	select pf.* 
	 ...
	 where DATA_ID = 124			--Total Cash Dividends Paid		
	...
	select b.ISSUER_ID, b.SECURITY_ID, a.COA_TYPE, a.DATA_SOURCE, a.ROOT_SOURCE
		...
		,  192 as DATA_ID										-- DATA_ID:192 Dividend Yield
		,  (-1*a.AMOUNT) /b.AMOUNT as AMOUNT						-- 124/185
		,  '124(' + CAST(-1*a.AMOUNT as varchar(32)) + ') / 185(' + CAST(b.AMOUNT as varchar(32)) + ')' as CALCULATION_DIAGRAM
		,  a.SOURCE_CURRENCY
		,  a.AMOUNT_TYPE
	  from #A a
