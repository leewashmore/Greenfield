...
as
	...
	select a.ISSUER_ID, a.SECURITY_ID, ' ', a.DATA_SOURCE, a.ROOT_SOURCE
		...
		,  289 as DATA_ID										-- 289 Previous Tax Rate
		,  a.AMOUNT as AMOUNT						-- Previous Annual Cash and Short Term Investments (51)*
		,  'Previous Tax Rate(' + CAST(a.AMOUNT as varchar(32)) + ')' as CALCULATION_DIAGRAM
		,  a.SOURCE_CURRENCY
		,  a.AMOUNT_TYPE
	  from #A a	
	 where 1=1 	  
	...
	if @CALC_LOG = 'Y'
		BEGIN	
			...
			select GETDATE() as LOG_DATE, 289 as DATA_ID, a.ISSUER_ID, 'C'
				...
				, 'ERROR calculating 289 Previous Tax Rate. DATA_ID:232 is NULL or ZERO'
			  from #A a
			...
			select GETDATE() as LOG_DATE, 289 as DATA_ID, isnull(@ISSUER_ID, ' ') as ISSUER_ID, ' ' as PERIOD_TYPE
				...
				, 'ERROR calculating 289 Prrevious Tax Rate.  DATA_ID:51 no data' as TXT
