------------------------------------------------------------------------
-- Purpose:	This procedure calculates the value for DATA_ID:231 Sustainable Growth
--
--			(NINC/ (QTLE + QTLE for Prior Period)/2) x (1 + (FCDP/NINC))
--
-- Author:	Shivani
-- Date:	July 19, 2012
------------------------------------------------------------------------
...
as
	...
	select a.ISSUER_ID, a.SECURITY_ID, a.COA_TYPE, a.DATA_SOURCE, a.ROOT_SOURCE
		...
		,  ((a.amount/(b.amount + c.amount)/2) * (1 + (d.amount/a.amount)))as AMOUNT						-- (NINC/ (QTLE + QTLE for Prior Period)/2) x (1 + (FCDP/NINC))
		,  '((NINC(' + CAST(a.AMOUNT as varchar(32)) + '/(QTLE(' + CAST(b.AMOUNT as varchar(32)) + ') + QTLE prior_yr ('+ CAST(c.AMOUNT as varchar(32)) +')) * (1 + (FCDP(' + CAST(d.AMOUNT as varchar(32))+ ') / NINC ( ' + CAST(a.AMOUNT as varchar(32)) + '))))'  as CALCULATION_DIAGRAM
		,  a.SOURCE_CURRENCY
		,  a.AMOUNT_TYPE
	  from #A a
	...
	if @CALC_LOG = 'Y'
		BEGIN	
			...
			select GETDATE() as LOG_DATE, 231 as DATA_ID, a.ISSUER_ID, a.PERIOD_TYPE
			 ...
			 select GETDATE() as LOG_DATE, 231 as DATA_ID, a.ISSUER_ID, a.PERIOD_TYPE
				,  a.PERIOD_YEAR, a.PERIOD_END_DATE, a.FISCAL_TYPE, a.CURRENCY
				, 'ERROR calculating 231 Sustainable Growth.  DATA_ID:44 NINC is NULL or ZERO'
			  from #A a
			...
			select GETDATE() as LOG_DATE, 231 as DATA_ID, a.ISSUER_ID, a.PERIOD_TYPE
				,  a.PERIOD_YEAR, a.PERIOD_END_DATE, a.FISCAL_TYPE, a.CURRENCY
				, 'ERROR calculating 231 Sustainable Growth.  DATA_ID:44 NINC is missing'
			  from #A a
			...
			select GETDATE() as LOG_DATE, 231 as DATA_ID, isnull(@ISSUER_ID, ' ') as ISSUER_ID, ' ' as PERIOD_TYPE
				...
				, 'ERROR calculating 231 Sustainable Growth.  DATA_ID:44 NINC no data' as TXT
