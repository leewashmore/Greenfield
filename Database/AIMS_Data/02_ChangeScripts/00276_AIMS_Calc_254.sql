...
declare @RequiredDBVersion as nvarchar(100) = '00275'
declare @CurrentScriptVersion as nvarchar(100) = '00276'
...
IF OBJECT_ID ( 'AIMS_Calc_254', 'P' ) IS NOT NULL 
DROP PROCEDURE AIMS_Calc_254;
GO


------------------------------------------------------------------------
-- Purpose:	This procedure calculates the value for DATA_ID:254  Forward P/E Relative to Country Industry
-- 187 / Forward P/E for Country Industry of Security*
-- Author:	Prerna
-- Date:	Aug 22, 2012
------------------------------------------------------------------------

Create procedure [dbo].[AIMS_Calc_254](
...
as
	...
	select distinct a.ISSUER_ID, a.SECURITY_ID, a.COA_TYPE, a.DATA_SOURCE, a.ROOT_SOURCE
		...
		,  254 as DATA_ID							-- DATA_ID:254  Forward P/E Relative to Country Industry
		,  a.AMOUNT / b.AMOUNT as AMOUNT			-- 187 / Forward P/E for Country Industry of Security*
	...
	if @CALC_LOG = 'Y'
		BEGIN	
			...
			select GETDATE() as LOG_DATE, 254 as DATA_ID, a.ISSUER_ID, a.PERIOD_TYPE
				,  a.PERIOD_YEAR, a.PERIOD_END_DATE, a.FISCAL_TYPE, a.CURRENCY
				, 'ERROR calculating  254 :Forward P/E Relative to Country Industry.  Forward P/E is missing' as TXT
			  from #A a
			...
			select GETDATE() as LOG_DATE, 254 as DATA_ID, isnull(@ISSUER_ID, ' ') as ISSUER_ID, ' ' as PERIOD_TYPE
				,  0 as PERIOD_YEAR,  '1/1/1900' as PERIOD_END_DATE,  ' ' as FISCAL_TYPE,  ' ' as CURRENCY
				, 'ERROR calculating 254: Forward P/E Relative to Country Industry.  DATA_ID:187 no data' as TXT
			  from (select COUNT(*) CNT from #A having COUNT(*) = 0) z
			) union	(
			select GETDATE() as LOG_DATE, 254 as DATA_ID, isnull(@ISSUER_ID, ' ') as ISSUER_ID, ' ' as PERIOD_TYPE
				,  0 as PERIOD_YEAR,  '1/1/1900' as PERIOD_END_DATE,  ' ' as FISCAL_TYPE,  ' ' as CURRENCY
				, 'ERROR calculating 254: Forward P/E Relative to Country Industry.  Forward P/E no data' as TXT
