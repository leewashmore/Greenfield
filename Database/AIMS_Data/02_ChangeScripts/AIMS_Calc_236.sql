...
as
 ...
 ---- Total amount for all the fiscal quarters within an year --- 
	...
	select distinct a.ISSUER_ID, a.SECURITY_ID, a.COA_TYPE, a.DATA_SOURCE, a.ROOT_SOURCE
		...
		,  b.AMOUNT /a.AMOUNT as AMOUNT						-- Sum of next 4 quarters FCDP**/Current Market Cap*
	...
	if @CALC_LOG = 'Y'
		BEGIN
			...
			select GETDATE() as LOG_DATE, 236 as DATA_ID, a.ISSUER_ID, 'C'
				...
				, 'ERROR calculating 236 Forward Dividend Yield. DATA_ID:185 Market Cap is NULL or ZERO'
			  from #A a
			...
			select GETDATE() as LOG_DATE, 236 as DATA_ID, a.ISSUER_ID, 'C'
				,  0,'01/01/1900', a.FISCAL_TYPE, a.CURRENCY
				, 'ERROR calculating Forward Dividend Yield.  DATA_ID:124 FCDP is missing' as TXT
			 from #A a
			  inner join dbo.GF_SECURITY_BASEVIEW sb on sb.SECURITY_ID = a.SECURITY_ID
			...
			select GETDATE() as LOG_DATE, 236 as DATA_ID, isnull(@ISSUER_ID, ' ') as ISSUER_ID, ' ' as PERIOD_TYPE
				...
				, 'ERROR calculating 236 Forward Dividend Yield.  DATA_ID:185 Market Cap no data' as TXT
			  from (select COUNT(*) CNT from #A having COUNT(*) = 0) z
			) union (	

			select GETDATE() as LOG_DATE, 236 as DATA_ID, isnull(@ISSUER_ID, ' ') as ISSUER_ID, ' ' as PERIOD_TYPE
				,  0 as PERIOD_YEAR,  '1/1/1900' as PERIOD_END_DATE,  ' ' as FISCAL_TYPE,  ' ' as CURRENCY
				, 'ERROR calculating 236 Forward Dividend Yield.  DATA_ID:124 FCDP no data' as TXT
