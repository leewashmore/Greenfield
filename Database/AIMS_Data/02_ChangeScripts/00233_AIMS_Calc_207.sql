...
declare @RequiredDBVersion as nvarchar(100) = '00232'
declare @CurrentScriptVersion as nvarchar(100) = '00233'
...
--
--			Current Market Cap* / Sum of previous 4 quarters NINC**
--
-- Author:	Anupriya
-- Date:	July 18, 2012
------------------------------------------------------------------------
...
as
 ...
 ---- Total amount for all the fiscal quarters within an year --- 
	...
	select distinct a.ISSUER_ID, a.SECURITY_ID, a.COA_TYPE, a.DATA_SOURCE, a.ROOT_SOURCE
		...
		,  a.AMOUNT /b.AMOUNT as AMOUNT						-- Current Market Cap* / Sum of previous 4 quarters NINC**
	...
	if @CALC_LOG = 'Y'
		BEGIN
			...
			select GETDATE() as LOG_DATE, 207 as DATA_ID, a.ISSUER_ID, 'C'
				...
				, 'ERROR calculating 207 Trailing P/E. DATA_ID:44 NINC is NULL or ZERO'
			   from #B a
			...
			select GETDATE() as LOG_DATE, 207 as DATA_ID, a.ISSUER_ID, 'C'
				,0, '01/01/1900', a.FISCAL_TYPE, a.CURRENCY
				, 'ERROR calculating 207 Trailing P/E .  DATA_ID:44 NINC is missing' as TXT
			  from #A a
			  inner join dbo.GF_SECURITY_BASEVIEW sb on sb.SECURITY_ID = a.SECURITY_ID
			...
			select GETDATE() as LOG_DATE, 207 as DATA_ID, isnull(@ISSUER_ID, ' ') as ISSUER_ID, ' ' as PERIOD_TYPE
				,  0 as PERIOD_YEAR,  '1/1/1900' as PERIOD_END_DATE,  ' ' as FISCAL_TYPE,  ' ' as CURRENCY
				, 'ERROR calculating 207 Trailing P/E.  DATA_ID:44 NINC No data or missing quarters' as TXT
