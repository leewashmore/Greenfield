...
declare @RequiredDBVersion as nvarchar(100) = '00199'
declare @CurrentScriptVersion as nvarchar(100) = '00200'
...
-- Date:	July 15, 2012
------------------------------------------------------------------------
IF OBJECT_ID ( 'AIMS_Calc_170', 'P' ) IS NOT NULL 
DROP PROCEDURE AIMS_Calc_170;
GO

CREATE procedure [dbo].[AIMS_Calc_170](
...
as
	...
	select pf.* 
	  into #B
	  from dbo.PERIOD_FINANCIALS  pf 
	 where DATA_ID = 11			-- Total Revenue	   
	...
	select a.ISSUER_ID, a.SECURITY_ID, a.COA_TYPE, a.DATA_SOURCE, a.ROOT_SOURCE
		...
		,  170 as DATA_ID										-- DATA_ID:170 P/Revenue  
		,  a.AMOUNT /b.AMOUNT as AMOUNT						-- 185/11
		,   '185(' + CAST(a.AMOUNT as varchar(32)) + ') / 11(' + CAST(b.AMOUNT as varchar(32)) + ')' as CALCULATION_DIAGRAM
		,  a.SOURCE_CURRENCY
		,  a.AMOUNT_TYPE
	  from #A a
	...
	if @CALC_LOG = 'Y'
		BEGIN
			...
			select GETDATE() as LOG_DATE, 170 as DATA_ID, a.ISSUER_ID, a.PERIOD_TYPE
				...
				, 'ERROR calculating 170 P/Revenue.  DATA_ID:11 is NULL or ZERO'
			  from #B a
			...
			select GETDATE() as LOG_DATE, 170 as DATA_ID, a.ISSUER_ID, a.PERIOD_TYPE
				,  a.PERIOD_YEAR,  a.PERIOD_END_DATE,  a.FISCAL_TYPE,  a.CURRENCY
				, 'ERROR calculating 170 P/Revenue.  DATA_ID:11 is missing' as TXT
			  from #B a
			  inner join dbo.GF_SECURITY_BASEVIEW sb on sb.SECURITY_ID = a.SECURITY_ID
			...
			select GETDATE() as LOG_DATE, 170 as DATA_ID, isnull(@ISSUER_ID, ' ') as ISSUER_ID, ' ' as PERIOD_TYPE
				,  0 as PERIOD_YEAR,  '1/1/1900' as PERIOD_END_DATE,  ' ' as FISCAL_TYPE,  ' ' as CURRENCY
				, 'ERROR calculating 170 P/Revenue.  DATA_ID:11 no data' as TXT
