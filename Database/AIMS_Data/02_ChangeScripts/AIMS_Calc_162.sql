...
as
	...
	select c.ISSUER_ID, c.SECURITY_ID, c.COA_TYPE, c.DATA_SOURCE, c.ROOT_SOURCE
		...
		,  (isnull(a.AMOUNT, 0.0) -  isnull(b.AMOUNT, 0.0)) / c.AMOUNT as AMOUNT			-- (NINC-FCDP)/QTEL
		,  '(NINC(' + CAST(isnull(a.AMOUNT, 0.0) as varchar(32)) + ') -  FCDP(' + CAST(isnull(b.AMOUNT, 0.0) as varchar(32)) + ')) / QTEL('  + CAST(c.AMOUNT as varchar(32)) + ') ' as CALCULATION_DIAGRAM
		,  c.SOURCE_CURRENCY
		,  c.AMOUNT_TYPE
	  from #C c
	...
	if @CALC_LOG = 'Y'
		BEGIN
			...
			select GETDATE() as LOG_DATE, 162 as DATA_ID, a.ISSUER_ID, a.PERIOD_TYPE
				,  a.PERIOD_YEAR,  a.PERIOD_END_DATE,  a.FISCAL_TYPE,  a.CURRENCY
				, 'ERROR calculating 162:ROIC.  DATA_ID:44 NINC is missing' as TXT
			  from #C a
			...
			select GETDATE() as LOG_DATE, 162 as DATA_ID, a.ISSUER_ID, a.PERIOD_TYPE
				,  a.PERIOD_YEAR,  a.PERIOD_END_DATE,  a.FISCAL_TYPE,  a.CURRENCY
				, 'ERROR calculating 162:ROIC.  DATA_ID:124 NINC is missing' as TXT
			  from #C a
			...
			select GETDATE() as LOG_DATE, 162 as DATA_ID, isnull(@ISSUER_ID, ' ') as ISSUER_ID, ' ' as PERIOD_TYPE
				...
				, 'ERROR calculating 162:ROIC.  DATA_ID:44 NINC no data' as TXT
			  from (select COUNT(*) CNT from #A having COUNT(*) = 0) z
			) union (
			select GETDATE() as LOG_DATE, 162 as DATA_ID, isnull(@ISSUER_ID, ' ') as ISSUER_ID, ' ' as PERIOD_TYPE
				,  0 as PERIOD_YEAR,  '1/1/1900' as PERIOD_END_DATE,  ' ' as FISCAL_TYPE,  ' ' as CURRENCY
				, 'ERROR calculating 162:ROIC.  DATA_ID:124 FCDP no data' as TXT
			  from (select COUNT(*) CNT from #B having COUNT(*) = 0) z
			) union (
			select GETDATE() as LOG_DATE, 162 as DATA_ID, isnull(@ISSUER_ID, ' ') as ISSUER_ID, ' ' as PERIOD_TYPE
				,  0 as PERIOD_YEAR,  '1/1/1900' as PERIOD_END_DATE,  ' ' as FISCAL_TYPE,  ' ' as CURRENCY
				, 'ERROR calculating 162:ROIC.  DATA_ID:105 QTEL no data' as TXT
