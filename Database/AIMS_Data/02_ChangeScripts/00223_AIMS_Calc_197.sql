...
declare @RequiredDBVersion as nvarchar(100) = '00222'
declare @CurrentScriptVersion as nvarchar(100) = '00223'
...
GO
            ------------------------------------------------------------------------
-- Purpose:	This procedure calculates the value for DATA_ID:197 Forward P/Revenue
--
--			Current Market Cap* / Sum of next 4 quarters RTLR**
--
-- Author:	Shivani 
-- Date:	July 17, 2012
------------------------------------------------------------------------
IF OBJECT_ID ( 'AIMS_Calc_197', 'P' ) IS NOT NULL 
DROP PROCEDURE AIMS_Calc_197;
GO

CREATE procedure [dbo].[AIMS_Calc_197](
...
as
 ...
 ---- Total amount for all the fiscal quarters within an year --- 
	...
	select distinct a.ISSUER_ID, a.SECURITY_ID, a.COA_TYPE, a.DATA_SOURCE, a.ROOT_SOURCE
		...
		,  197 as DATA_ID										-- DATA_ID:197 Forward P/Revenue
	...
	if @CALC_LOG = 'Y'
		BEGIN
			...
			select GETDATE() as LOG_DATE, 197 as DATA_ID, a.ISSUER_ID, 'C'
				...
				, 'ERROR calculating 197 Forward P/Revenue. DATA_ID:11 RTLR is NULL or ZERO'
			  from #B a
			...
			select GETDATE() as LOG_DATE, 197 as DATA_ID, a.ISSUER_ID, 'C'
				,  0, '01/01/1900', a.FISCAL_TYPE, a.CURRENCY
				, 'ERROR calculating 197 Forward P/Revenue .  DATA_ID:185 is missing' as TXT
			   from #A a
			  inner join dbo.GF_SECURITY_BASEVIEW sb on sb.SECURITY_ID = a.SECURITY_ID
			...
			select GETDATE() as LOG_DATE, 197 as DATA_ID, isnull(@ISSUER_ID, ' ') as ISSUER_ID, ' ' as PERIOD_TYPE
				,  0 as PERIOD_YEAR,  '1/1/1900' as PERIOD_END_DATE,  ' ' as FISCAL_TYPE,  ' ' as CURRENCY
				, 'ERROR calculating 197 Forward P/Revenue .  DATA_ID:185 no data' as TXT
			  from (select COUNT(*) CNT from #A having COUNT(*) = 0) z
			) union (	

			select GETDATE() as LOG_DATE, 197 as DATA_ID, isnull(@ISSUER_ID, ' ') as ISSUER_ID, ' ' as PERIOD_TYPE
				,  0 as PERIOD_YEAR,  '1/1/1900' as PERIOD_END_DATE,  ' ' as FISCAL_TYPE,  ' ' as CURRENCY
				, 'ERROR calculating 197 Forward P/Revenue.  DATA_ID:11 RTLR no data or missing quarters' as TXT
...
-- exec AIMS_Calc_197 117567
-- exec AIMS_Calc_0 117567
-- select * from calc_log
