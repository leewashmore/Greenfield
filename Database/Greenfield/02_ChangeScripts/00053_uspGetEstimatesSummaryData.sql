...
SET @earnings = (SELECT Earnings FROM dbo.tblCompanyInfo
WHERE XRef = @xref)

--SELECT @earnings AS EARNINGS

SET @currency = (SELECT Currency FROM DBO.tblCompanyInfo
 WHERE XRef  = @xref)
...
(SELECT CONVERT(VARCHAR(100),DATE_SOURCE_DATE,121) FROM @FINAL_TAB WHERE FISCAL_YEAR = @currentYear AND MTYPE ='SourceDate'),
(SELECT CONVERT(VARCHAR(100),DATE_SOURCE_DATE,121) FROM @FINAL_TAB WHERE FISCAL_YEAR = @currentYear+1 AND MTYPE ='SourceDate'),
(SELECT CONVERT(VARCHAR(100),DATE_SOURCE_DATE,121) FROM @FINAL_TAB WHERE FISCAL_YEAR = @currentYear+2 AND MTYPE ='SourceDate'),
(SELECT CONVERT(VARCHAR(100),DATE_SOURCE_DATE,121) FROM @FINAL_TAB WHERE FISCAL_YEAR = @currentYear+3 AND MTYPE ='SourceDate')
